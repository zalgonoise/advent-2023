...
func (m *Matrix) NearSymbol(distance int) []CoordRange {
	...
	for idx := range m.symbols {
		switch {
		case m.values[m.symbols[idx].Y][m.symbols[idx].X] == '*':
			matches := make([]CoordRange, 0, 2)
			...
			for i := range m.numbers {
				...
				if valid.XStart-distance <= m.symbols[idx].X && valid.XEnd-1+distance >= m.symbols[idx].X {
					matches = append(matches, m.numbers[i])
				}
			}
			...
			case 0:
				continue
			case 1:
				nearby = append(nearby, matches[0])
			default:
				for i := len(matches) - 1; i > 0; i-- {
					matches[i].Gear = &matches[i-1]
				}

				nearby = append(nearby, matches[len(matches)-1])
			}
...
func (m *Matrix) Sum(ranges []CoordRange) (int, error) {
	...
	for i := range ranges {
		value, ok, err := m.rangeAsInt(ranges[i])
		if err != nil {
...
func (m *Matrix) rangeAsInt(r CoordRange) (int, bool, error) {
	if r.Y >= len(m.values) {
		return -1, false, fmt.Errorf("%w: %d:%d", errYAxisOverflow, r.XStart, r.Y)
	}

	if r.XStart >= len(m.values[r.Y]) {
		return -1, false, fmt.Errorf("%w: %d:%d", errXAxisOverflow, r.XStart, r.Y)
	}

	if r.XEnd > len(m.values[r.Y]) {
		return -1, false, fmt.Errorf("%w: %d:%d", errXAxisOverflow, r.XEnd, r.Y)
	}
	...
	inner, _, err := m.rangeAsInt(*r.Gear)
	if err != nil {
		return -1, false, err
	}

	return n * inner, true, nil
}
...
func numbers(values [][]byte) []CoordRange {
	...
	for row, value := range values {
		...
		for len(value) > 0 {
			...
			coords = append(coords, CoordRange{n + start, n + start + end, row, nil})
			n += start + end
		}
	}
