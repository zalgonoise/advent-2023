...
func Sum(g *Grid) int {
	caches := make([]map[Coord]map[Coord]struct{}, (g.maxY+1)*2+(g.maxX+1)*2)
	...
	slices.SortFunc(caches, func(a, b map[Coord]map[Coord]struct{}) int {
		return cmp.Compare(len(a), len(b))
	})

	return len(caches[len(caches)-1])
}
